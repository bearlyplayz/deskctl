...
/// </remarks>
[McpServerToolType]
internal sealed class WinDeskCtlTools
{
    private WinDeskCtlTools() { }
    ...
    [Description(
        "Self-test windeskctl against the live machine. Reports virtual-desktop bounds, every " +
    ...
    public static async Task<CallToolResult> RecordAsync(
        ...
        => await ReportingCallerErrors(async () =>
        {
            ...
            string structured = WinDeskCtlJson.Serialize(result with { Files = names });
    ...
    [Description(
        ...
        "VISIBLE edges — windeskctl converts to the raw rect Win32 wants, so what you ask for is " +
    ...
    public static async Task<InputResult> InputAsync(
        ...
        => await ReportingCallerErrors(() =>
        {
            ...
            // by type.
            List<Step>? parsed = WinDeskCtlJson.Deserialize<List<Step>>(raw);
