...
public sealed class WindowActionCommand : ICommand<WindowActionInput, WindowActionResult>
{
    ...
    /// the taskbar button.
    ///
    /// 1. SetForegroundWindow alone. Succeeds when deskctl already owns the foreground.
    ...
    ///    the calling thread that attaches — it is deskctl's privilege that is lacking, so
    ...
    private static void Focus(nint hwnd)
    {
        ...
        if (!BecameForeground(hwnd))
        {
            throw new InvalidOperationException(
                ...
                "deskctl from reaching, or the desktop may be showing a full-screen exclusive app.");
        }
    }
    ...
    private static void MoveOrResize(nint hwnd, WindowActionInput input, bool resize)
    {
        ...
        if (!User32.SetWindowPos(hwnd, 0, raw.OriginX, raw.OriginY, raw.W, raw.H, flags))
        {
            ...
            throw new InvalidOperationException(
                ...
                "It may be running elevated, which UIPI blocks deskctl from reaching.");
        }
    }
}
